...
#[tokio::test]
async fn test_account_summary_cancel_is_legacy_on_server_221() {
    let (client, message_bus) =
        create_test_client_with_responses_and_version(vec![account_summary().encode_pipe(), account_summary_end().encode_pipe()], 221);
    let group = AccountGroup("All".to_string());
    let mut subscription = client
